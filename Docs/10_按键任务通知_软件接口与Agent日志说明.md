...
> 历史说明：本文保留阶段3当时固定 `/mcu_dev`接口的联调记录；当前源码仍由按键动作入口向LED任务投递动作，但阶段5.2B已把ROS接口改为SN动态前缀。当前通信名称以源码、`Docs/08_阶段3_micro-ROS消息接口设计.md`和 `Docs/11_阶段5_Bootloader_SNTool_IAP实施记录.md`为准。
